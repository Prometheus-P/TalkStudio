...
- [X] T003 `frontend/src`를 `src`로 변경하는 작업 (선택 사항, 기존 React 프로젝트 구조 유지 시 필요) 또는 `frontend/` 디렉토리 생성 및 기존 `src` 이동
- [X] T004 `backend/` 디렉토리 내 Node.js (또는 Python) 프로젝트 초기화 및 기본 파일 (package.json 또는 pyproject.toml/requirements.txt) 생성
...
- [X] T007 `ai_agent_system/requirements.txt`에 `discord.py` 및 `upstage-api-python-sdk` (가칭) 종속성 추가
- [X] T008 `backend/package.json` (또는 Python 백엔드인 경우 `requirements.txt`)에 필요한 웹 프레임워크 (Express.js/Fastify) 및 데이터베이스 (MongoDB/PostgreSQL 클라이언트) 종속성 추가
...
- [X] T011 `backend/src/db/` 디렉토리 및 데이터베이스 연결 설정 (`NoSQL (MongoDB) 또는 관계형 (PostgreSQL)`) 구현
- [X] T012 `ai_agent_system/src/db/` 디렉토리 및 데이터베이스 연결 설정 구현 (Backend와 동일한 DB 사용)
...
- [X] T016 [P] [US1] `backend/src/models/discord_config_model.js` (또는 .py)에 `DiscordConfig` 데이터 모델 구현 및 데이터베이스 연동
- [X] T017 [P] [US1] `backend/src/api/integrations/discord_config_routes.js` (또는 .py)에 `POST /integrations/discord/config` (설정 등록/업데이트) API 엔드포인트 구현
- [X] T018 [P] [US1] `backend/src/api/integrations/discord_config_routes.js` (또는 .py)에 `GET /integrations/discord/config/{configId}` (설정 조회) API 엔드포인트 구현
- [X] T019 [P] [US1] `backend/src/api/integrations/discord_config_routes.js` (또는 .py)에 `DELETE /integrations/discord/config/{configId}` (설정 삭제) API 엔드포인트 구현
- [X] T020 [P] [US1] `backend/src/api/integrations/discord_capture_routes.js` (또는 .py)에 `POST /integrations/discord/config/{configId}/capture/start` (캡쳐 시작/재개) API 엔드포인트 구현 (AI Agent System에 작업 위임)
- [X] T021 [P] [US1] `backend/src/api/integrations/discord_capture_routes.js` (또는 .py)에 `GET /integrations/discord/capture/{captureJobId}/status` (캡쳐 상태 조회) API 엔드포인트 구현
...
- [X] T030 [P] [US2] `backend/src/api/integrations/intent_analysis_routes.js` (또는 .py)에 `GET /integrations/discord/messages/{discordMessageId}/intent-analysis` (의도 분석 결과 조회) API 엔드포인트 구현
...
- [X] T035 [P] [US3] `backend/src/api/content_generation_routes.js` (또는 .py)에 `POST /content/generate` (콘텐츠 생성 요청) API 엔드포인트 구현 (AI Agent System에 작업 위임)
- [X] T036 [P] [US3] `backend/src/api/content_generation_routes.js` (또는 .py)에 `GET /content/generated/{generatedContentId}` (생성된 콘텐츠 조회) API 엔드포인트 구현
...
- [X] T045 [US4] (선택 사항) `ai_agent_system/src/services/discord_notifier.py`에 Discord 채널로 메시지를 전송하는 로직 구현
